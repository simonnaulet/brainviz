...
class CompactUNet(nn.Module):
    ...
    def forward(self, x, plane=None):
        """Segmente une image 2D ou la coupe centrale d'un stack 2.5D.

        ``plane`` est accepté pour rendre le modèle compatible avec le pipeline
        tri-plan partagé, mais le U-Net n'applique aucun conditionnement par plan.
        """
        del plane
        if x.ndim == 5:
            if x.shape[2] % 2 == 0:
                raise ValueError("le stack doit contenir un nombre impair de coupes")
            x = x[:, :, x.shape[2] // 2]
        if x.ndim != 4:
            raise ValueError("CompactUNet attend [B,C,H,W] ou [B,C,S,H,W]")
        if x.shape[1] != self.in_channels:
            raise ValueError(f"attendu {self.in_channels} canaux, reçu {x.shape[1]}")
        skips = []
