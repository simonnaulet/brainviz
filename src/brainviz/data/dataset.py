...
class BrainSliceDataset(Dataset):
    ...
    Raises:
        ValueError: axis ou modality invalide, root_dir n'est pas un dossier, ou
            root_dir ne contient aucun sous-dossier de sujet.
    """

    def __init__(self, root_dir, axis=2, modality="T1", scaling="padding"):
        ...
        self.data = torch.cat(data_slices, dim=0)
...
def get_dataloader(root_dir, axis=2, modality="T1", scaling="padding", batch_size=32, shuffle=True, **dataloader_kwargs):
    ...
    Returns:
        torch.utils.data.DataLoader: dataloader sur BrainSliceDataset(root_dir, axis, modality, scaling).
    """
    dataset = BrainSliceDataset(root_dir, axis=axis, modality=modality, scaling=scaling)
